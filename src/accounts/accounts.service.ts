...
const mg = mailgun.client({username: 'api', key: process.env.MAILGUN_API_KEY || 'key-756gqpx19lzm2xkk$5'});
...
export class AccountsService {
  ...
  async addNewUser(body): Promise<any> {
    ...
    return {status: 'success', message: 'Signup is successful, you can now login'};
  }
  ...
  async resetPassword(body): Promise<any>{
    ...
    await this.userModel.update({ token: null, password: this.hashPassword(body.password) }, { where: { token:body.token } })
  ...
  async forgotPassword(email, origin): Promise<any> {
    ...
    if(!findEmail) throw  new HttpException(
      {
        status: 'error',
  ...
  let message;
  if (origin) {
      const resetUrl = `${origin}/reset-password?token=${account.token}`;
